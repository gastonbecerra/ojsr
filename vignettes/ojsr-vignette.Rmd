...
`process_urls()` is invoked from the `get_*_url_from_*` functions before scraping when `use_contentional_url = TRUE` (or default).
...
2. *issue* - the issues url



## get_article_urls_from_issue: Retrieve articles urls from issue pages

`get_article_urls_from_issue()` takes a vector of OJS urls pointing at the issues of a journal (e.g., <https://publicaciones.sociales.uba.ar/index.php/psicologiasocial/issue/view/319/showToc>) and scraps them to retrieve the links to articles.
...
2. *galley* - the galley url linked
3. *format* - the format of the galley (e.g., pdf, xml)
4. *force* - the conventional url to force download of the galley
