...
`process_urls()` parses a list of urls against OJS **routing conventions** (<https://docs.pkp.sfu.ca/dev/documentation/en/architecture-routes>) to extract the parameters invoked (e.g., article, issues, galley IDs) and generate the *conventional* urls for different OJS pages. `process_urls()` is invoked from `get_*_url_from_*` functions before and after scraping. On the inputs, it is used to get the url to

get the *conventional* url required for each scrapping purpose; on the outputs, to ensure the right type of page is served. 
